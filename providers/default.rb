...
def create_command_string(artifact_file, new_resource)
  ...
  settings_xml = '--settings ' + new_resource.settings_xml if new_resource.settings_xml
  disable_ssl_verify = '-Dmaven.wagon.http.ssl.insecure=True -Dmaven.wagon.http.ssl.allowall=true' if !new_resource.ssl_verify
  %Q{mvn #{disable_ssl_verify} #{settings_xml} #{plugin} #{group_id} #{artifact_id} #{version} #{packaging} #{classifier} #{dest} #{repos} #{transitive}}
end
